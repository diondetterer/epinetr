...
```{r fig.width=5, fig.asp=1, fig.align='center', fig.cap="An epistatic network generated between 50 QTLs."}
# Plot the network
plot(getEpiNet(pop))
```
...
```{r fig.width=7, fig.height=4, fig.align='center', fig.cap="A graphical representation of a simulation run across 250 generations."}
...
We can use the *scaleFree* flag to generate a network using the Barabasi-Albert model^[[Barabasi AL, Albert R, "Emergence of scaling in random networks," _Science_ 286(5439): 509-12, 15 October 1999.](http://barabasi.com/f/67.pdf)].

```{r results=FALSE}
pop <- attachEpiNet(pop, scaleFree = TRUE)
```

```{r fig.width=5, fig.asp=1, fig.align='center', fig.cap="An epistatic network generated using the Barabasi-Albert model between 20 QTLs."}
plot(getEpiNet(pop))
```
...
```{r, fig.width=5, fig.asp=1, fig.align='center', fig.cap="An epistatic network where 7 QTLs have no epistatic effects."}
plot(getEpiNet(pop))
```
...
```{r, fig.width=5, fig.asp=1, fig.align='center', fig.cap="An epistatic network with a minimum of 2 interactions per epistatic QTL."}
plot(getEpiNet(pop))
```

There are two points to note with the *m* argument. The first point is that it has no impact on QTLs designated as additive-only. The second point is that a minimal connected graph is initially constructed prior to *m* being strictly applied. (This means that you may still see QTLs with only a single interaction.)
...
```{r, fig.width=5, fig.asp=1, fig.align='center', fig.cap="An epistatic network featuring 2-way to 7-way interactions"}
plot(getEpiNet(pop))
```

In cases where $m > 1$, any initial minimal connected graph is created by selecting $n$ nodes such that $n$ is the smallest integer satisfying the equation $\binom{n}{k - 1} \ge m$.
...
Thus the contribution of this particular interaction to this individual's overall phenotype is `r interaction1[2, 1, 3]`.
...
Alternative, we could use the *getHaplo* function, which returns a list of the two haplotype matrices within the population; we would then need to sum the two haplotypes together.
...
The rows in this matrix are the individuals; the columns are the contributions of each interaction to the overall epistatic component. By summing the rows and applying the epistatic offset to each value, we can derive the contribution of epistatis to each individual's phenotype:
...
We can thus easily derive both the additive and epistatic components for each individual.
...
Finally, we can use a pedigree data frame to determine selection. This "pedigree dropper" first sorts the pedigree into the implicit number of generations; note that this may be different from the number of iterations of the simulator that produced the pedigree.
